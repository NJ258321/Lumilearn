...
const CONFIG = {
    // Layout - More Compact
    LEVEL_X_OFFSET: [0, 200, 380, 580], // Even tighter X spacing
    MACRO_GAP_Y: 70, 
    MICRO_GAP_Y: 55, 
...
const CourseDetailStudy: React.FC<CourseDetailStudyProps> = ({ onNavigate }) => {
  ...
  const handleResetView = () => {
    ...
    if (containerRef.current && treeHeight > 0) {
        ...
        const desiredHeight = treeHeight + 180; 
        let scale = clientHeight / desiredHeight;
        scale = Math.min(Math.max(scale, 0.5), 0.85);
        ...
        // Shift overall layout to the right (increased from 40 to 80)
        // L1 is at x=200. With 80 padding, Center of L1 will be at 80px from left edge + negative offset
        // This pushes the whole tree to the right.
        const initialX = 80 - (CONFIG.LEVEL_X_OFFSET[1] * scale);
  ...
  const focusChapter = (nodeId: string) => {
      ...
      if (node && containerRef.current) {
          ...
          // 使用更小的缩放比例，确保L3节点完全可见
          const targetScale = 0.7; 
          
          // Center the node vertically
          const targetY = (clientHeight / 2) - (node.y * targetScale);

          // 水平定位：更靠左，确保L3节点不超出屏幕
          // 计算L3节点的最大x坐标（约580px）
          const maxChildX = CONFIG.LEVEL_X_OFFSET[3] + 180; // 180是节点宽度估计值
          const targetX = 160 - (node.x * targetScale); // 更靠左的偏移量
          
          // 确保右侧不会超出屏幕
          const rightEdge = (node.x + maxChildX) * targetScale + targetX;
          const adjustedX = rightEdge > clientWidth 
              ? targetX - (rightEdge - clientWidth) * 0.8 
              : targetX;
          
          setTransform({ x: adjustedX, y: targetY, scale: targetScale });
  ...
  return (
    <div className="h-screen w-full bg-[#F1F5F9] overflow-hidden relative font-sans text-slate-800 flex flex-col">
                ...
                {nodes.map(node => {
                    ...
                    return (
                        <div 
                            ...
                            onDoubleClick={() => node.type === 'chapter' && focusChapter(node.id)}
                        >
                            {/* Root Node */}
                            {node.type === 'root' && (
                            ...
                            {(node.type === 'chapter' || node.type === 'leaf') && (
                                 <div className={`relative px-4 py-3 rounded-xl shadow-sm min-w-[180px] border transition-all duration-500 flex items-center justify-between group cursor-pointer ${
                                    isLocked
        ...
        >
            <button 
                onClick={() => onNavigate(AppView.RECORDER, { courseName: '遥感原理与应用' })}
