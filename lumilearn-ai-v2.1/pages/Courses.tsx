...
const CONFIG = {
    ...
    MACRO_GAP_Y: 80,
    MICRO_GAP_Y: 48,
    // Pushed further down to accomodate the top graph comfortably
    TIMELINE_START_Y: 520, 

    // Force Graph Constants (Adjusted for Mobile Width ~375px)
    FORCE_CENTER_Y: 180, // Position relative to the top of the canvas
    FORCE_CENTER_X: 190, // Roughly center of a mobile screen
};
...
const Courses: React.FC<CoursesProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation(nodes as any)
        ...
        .force("x", d3.forceX((d: any) => {
            ...
            if (d.group === 'other') return CONFIG.FORCE_CENTER_X + 130; // Far Right
            return CONFIG.FORCE_CENTER_X; // Core stays center
        }).strength(0.8)) // Stronger pull to keep them in columns
        .force("y", d3.forceY((d: any) => {
            if (d.group === 'other') return CONFIG.FORCE_CENTER_Y + 120; // Bottom
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC] relative overflow-hidden font-sans">
                             ...
                             {forceNodes.map((node, i) => {
                                 ...
                                 return (
                                     <g 
                                        ...
                                        onClick={() => onNavigate(node.status === 'reviewing' ? AppView.COURSE_DETAIL_REVIEW : AppView.COURSE_DETAIL_STUDY, node.id)}
        ...
        {/* --- FLOATING LEGEND (Light Theme) --- */}
        {!isPanelOpen && (
            <div className="absolute top-20 right-4 bg-white/90 backdrop-blur-md border border-slate-100 p-3 rounded-2xl shadow-lg z-30 flex flex-col space-y-3 animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-1 mb-1">Status</div>
                <div className="flex items-center space-x-2">
                    <div className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.3)]"></div>
                    <span className="text-[10px] font-bold text-slate-600">Studying</span>
                </div>
                <div className="flex items-center space-x-2">
                    <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.3)]"></div>
                    <span className="text-[10px] font-bold text-slate-600">Reviewing</span>
                </div>
                <div className="flex items-center space-x-2">
                ...
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-1 mb-1 pt-2">Relations</div>
                <div className="flex items-center space-x-2">
                    <div className="w-6 h-0.5 bg-slate-400"></div>
                    <span className="text-[10px] text-slate-400">理论支撑</span>
                </div>
                <div className="flex items-center space-x-2">
                    <div className="w-6 h-0.5 border-t-2 border-dashed border-slate-400"></div>
                    <span className="text-[10px] text-slate-400">方法支撑</span>
                </div>
                <div className="flex items-center space-x-2">
                    <div className="w-6 h-0.5 border-t-2 border-dotted border-slate-400"></div>
                    <span className="text-[10px] text-slate-400">应用关联</span>
                </div>
            </div>
        )}
