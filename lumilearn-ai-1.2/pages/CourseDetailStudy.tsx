...
import { ArrowLeft, Edit3, Trash2, Calendar, MapPin, CheckCircle2, RotateCcw, Zap, Mic, Plus, Lock, MoreHorizontal, BookOpen, FileText, Settings, Minus } from 'lucide-react';
...
const CONFIG = {
    // Layout
    LEVEL_X_OFFSET: [0, 260, 540, 800], // Increased spacing for better label fitting
    MACRO_GAP_Y: 70, // Tighter for overview
    MICRO_GAP_Y: 55, // Spaced for leaves
    
    // Interaction
    SHOW_LEAF_THRESHOLD: 1.0, // Scale >= 1.0 shows leaves
...
const CourseDetailStudy: React.FC<CourseDetailStudyProps> = ({ onNavigate }) => {
  ...
  const [transform, setTransform] = useState({ x: 0, y: 0, scale: 0.8 }); // Start zoomed out
  ...
  const handleResetView = () => {
    if (containerRef.current && treeHeight > 0) {
        ...
        const desiredHeight = treeHeight + 150; 
        let scale = clientHeight / desiredHeight;
        scale = Math.min(Math.max(scale, 0.5), 0.9); // Keep it macro

        const treeCenterY = treeHeight / 2;
        const screenCenterY = clientHeight / 2;
        const initialY = screenCenterY - (treeCenterY * scale);
        // Position L1 (260) near the left edge (e.g. 40px padding)
        const initialX = 40 - (CONFIG.LEVEL_X_OFFSET[1] * scale);
  ...
  const { nodes, links, spine, treeHeight } = useMemo(() => {
    ...
    const traverse = (data: any, level: number, parentNode?: VisualNode) => {
        // SEMANTIC ZOOM: Hide Leaves (L3) if in Macro Mode
        ...
        // EDIT MODE: Inject "Add Node" Placeholder at end of Chapter children
        if (isEditMode && isMicro && level === 2 && data.status !== 'locked') {
        ...
        if (childNodes.length === 0) {
            node.y = currentY;
            // Variable Gap logic
            currentY += (level === 2 && isMicro) ? CONFIG.MICRO_GAP_Y : CONFIG.MACRO_GAP_Y;
        } else {
            // Fix for Placeholder Y: Manually assign Y if it wasn't traversed
  ...
  const focusNode = (nodeId: string) => {
      ...
      if (node && containerRef.current) {
          ...
          // Ensure we are in Micro mode to see details if focusing a leaf/chapter
          const targetScale = Math.max(transform.scale, 1.1); 
          
          const targetX = clientWidth / 2 - node.x * targetScale;
  ...
  return (
    <div className="h-screen w-full bg-[#F1F5F9] overflow-hidden relative font-sans text-slate-800 flex flex-col">
        ...
        <div 
            ...
            style={{ bottom: sheetHeight }} // Binds canvas bottom to sheet top
        ...
        >
            ...
            <div 
                ...
                style={{ 
                    ...
                    transition: (isDragging || evCache.current.size > 0) ? 'none' : 'transform 0.15s linear'
                }}
            >
                ...
                {nodes.map(node => {
                    ...
                    return (
                        <div 
                            key={node.id}
                            className={`absolute flex items-center justify-center transform -translate-y-1/2 transition-all duration-300`}
                            style={{ 
                                left: node.x, 
                                top: node.y,
                                zIndex: isActive ? 20 : 10,
                                transform: `translateY(-50%) scale(${isHighlighted ? 1.15 : 1})`
                            }}
                        >
                            {/* Root Node */}
                            {node.type === 'root' && (
                            ...
                            {(node.type === 'chapter' || node.type === 'leaf') && (
                                <div className={`relative px-4 py-3 rounded-xl shadow-sm min-w-[180px] border transition-all flex items-center justify-between group cursor-pointer ${
                                    isLocked
            ...
            <div className="flex-1 overflow-hidden flex">
                ...
                <div className="w-1/2 h-full overflow-y-auto border-r border-slate-100 px-4 py-4">
                    ...
                    <div className="relative border-l-2 border-slate-100 ml-2 space-y-6 pb-20">
                        {[
                            { id: 'sess-1', date: '10.08', title: '电磁波谱', relatedNodeId: 'ch-1' },
                            { id: 'sess-2', date: '10.10', title: '卫星轨道', relatedNodeId: 'ch-2' },
                            { id: 'sess-3', date: '10.12', title: '成像原理', relatedNodeId: 'ch-3' },
                            { id: 'sess-4', date: '10.15', title: '几何处理', relatedNodeId: 'ch-5', active: true },
                        ].map((sess) => (
                            <div 
                                key={sess.id} 
                                onClick={() => focusNode(sess.relatedNodeId)}
                                className={`relative pl-4 cursor-pointer group transition-all`}
                            >
                                <div className={`absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full border-2 transition-colors ${sess.active ? 'bg-emerald-500 border-emerald-200' : 'bg-white border-slate-300'}`}></div>
                                <div>
