...
async function calculateCorrelation(
  sourceId: string,
  targetId: string
): Promise<{ correlation: number; reason: string }> {
  let correlation = 0
  ...
  const similarQuestions = targetMistakes.filter(m =>
    Array.from(sourceKeywords).some(k => m.question.includes(k))
  )
...
router.get('/analysis/knowledge-points/:courseId/correlation', [
  ...
  try {
    ...
    const existingPairs = new Set(
      relations.map(r => `${r.sourceId}-${r.targetId}`)
    )
    ...
    for (let i = 0; i < knowledgePoints.length && additionalCount < limit; i++) {
      for (let j = i + 1; j < knowledgePoints.length && additionalCount < limit; j++) {
        ...
        if (!existingPairs.has(pairKey) && !existingPairs.has(reversePairKey)) {
          // 计算潜在关联度
          const { correlation, reason } = await calculateCorrelation(
            knowledgePoints[i].id,
            knowledgePoints[j].id
          )
    ...
    // 按章节分组
    const chapterGroups = new Map<string, typeof knowledgePoints>()
...
router.get('/analysis/learning-sequence/:courseId', [
  ...
  try {
    ...
    // 计算每个知识点需要的预估时间
    const estimatedTimeMap = new Map<string, number>()
    for (const kp of knowledgePoints) {
      // 基础时间：30分钟
      // 根据重要性调整：重要知识点多花时间
...
router.get('/analysis/bottlenecks/:courseId', [
  ...
  try {
    ...
    // 构建前置依赖图
    const prereqMap = new Map<string, typeof prerequisites>()
    for (const prereq of prerequisites) {
      if (!prereqMap.has(prereq.targetId)) {
        prereqMap.set(prereq.targetId, [])
      }
    ...
    for (const kp of knowledgePoints) {
      ...
      const blockers: typeof bottlenecks[0]['blockers'] = []
      const reasons: string[] = []
  ...
  try {
    ...
    for (const [hour, duration] of hourDistribution) {
      if (duration > maxTime) {
        ...
        else if (hour >= 12 && hour < 14) preferredTime = 'afternoon'
        else if (hour >= 14 && hour < 18) preferredTime = 'late_afternoon'
        else if (hour >= 18 && hour < 22) preferredTime = 'evening'
    ...
    const efficiencyScore = Math.round(
      (timePerPoint > 0 ? Math.max(0, 100 - timePerPoint * 2) : 50) * 0.3 +
