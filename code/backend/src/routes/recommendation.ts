 ...
 * 基于：掌握度、重要性、遗忘曲线、错题率
 */
async function calculateRecommendationScore(knowledgePointId: string): Promise<{
  score: number
  reasons: string[]
}> {
 ...
 * 获取用户的学习统计
 */
async function getUserLearningStats(userId: string | null): Promise<{
...
router.get('/recommendations/learning-path/:courseId', [
  ...
  try {
    ...
    scoredPoints.sort((a, b) => {
      if (a.canLearn && !b.canLearn) return -1
      if (!a.canLearn && b.canLearn) return 1
  ...
  try {
    ...
    if (itemType === 'knowledge_point' && action === 'complete') {
      ...
      if (kp) {
        ...
        else if (newScore >= 50) newStatus = 'LEARNING'
        else if (newScore > 0) newStatus = 'WEAK'
