...
type QuestionStatus = 'ACTIVE' | 'INACTIVE' | 'REVIEW'
...
router.get('/questions/export', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: '导出题目失败', code: 'EXPORT_QUESTIONS_FAILED' } as ApiResponse<undefined>)
  }
})
...
router.put('/questions/:id', [
  ...
  try {
    ...
    if (answer !== undefined) updateData.answer = JSON.stringify(answer)
