...
router.get('/recently-reviewed', [
  ...
  try {
    ...
    const recentlyReviewed = await prisma.knowledgePoint.findMany({
      where: courseId ? {
        chapter: { courseId: courseId as string }
      } : undefined,
      include: {
        chapter: {
...
router.post('/', [
  ...
  try {
    const { chapterId, name, status = 'NEED_REVIEW', importance = 5 }: CreateKnowledgePointRequest = req.body
...
router.put('/:id', [
  ...
  try {
    const { id } = req.params
    const updateData: UpdateKnowledgePointRequest = req.body
...
router.get('/weak', [], async (req: Request, res: Response) => {
  try {
...
// ==================== POST /api/knowledge-points/batch - 批量创建知识点 (Task-1.4.1) ====================

interface BatchCreateKnowledgePoint {
  name: string
  importance?: number
  masteryScore?: number
  status?: string
...
router.post('/batch', [
  ...
  try {
    const { chapterId, points } = req.body as {
      chapterId: string
      points: BatchCreateKnowledgePoint[]
    }
