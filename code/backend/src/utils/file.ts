 ...
 * @returns UUID文件名
 */
export function generateUUIDFilename(originalName: string): string {
  const ext = path.extname(originalName)
  const uuid = uuidv4()
  return `${uuid}${ext}`
}
 ...
 * @returns 文件类型分类
 */
export function getFileCategory(mimetype: string): 'audio' | 'image' | 'document' | 'unknown' {
 ...
 * @returns 是否允许
 */
export function isValidFileType(mimetype: string, category?: 'audio' | 'image' | 'document'): boolean {
  const fileCategory = getFileCategory(mimetype)
  if (!category) return fileCategory !== 'unknown'
 ...
 * @returns 文件信息
 */
export async function getFileInfo(filename: string): Promise<Stats | null> {
  try {
    const filePath = getFilePath(filename)
