...
- 重点关注：${focusAreas.join('、')}`
    ...
    if (weakPoints && weakPoints.length > 0) {
      prompt += `\n\n薄弱知识点：\n${weakPoints.map(wp =>
        `- ${wp.name} (掌握度：${wp.masteryScore}%，错题率：${(wp.mistakeRate * 100).toFixed(0)}%)`
      ).join('\n')}`
    }
...
高优先级（需要重点复习）：\n${highPriority.map(p =>
      `- ${p.knowledgePoint.name}：薄弱度${p.score.toFixed(0)}，错题率${(p.mistakeRate * 100).toFixed(0)}%${p.errorPatterns ? '，错误模式：' + p.errorPatterns.join('、') : ''}`
    ).join('\n') || '无'}`

    if (mediumPriority.length > 0) {
      prompt += `\n\n中优先级（需要适当关注）：\n${mediumPriority.map(p =>
        `- ${p.knowledgePoint.name}：薄弱度${p.score.toFixed(0)}`
      ).join('\n')}`
    }
...
3. 学习洞察和建议`
   ...
   */
  async searchResources(request: SearchResourcesRequest): Promise<ResourceResult[]> {
...
格式要求：JSON数组，每个元素包含 title, source, reason, type 字段`
  ...
  private getMockResponse(prompt: string, type: string): string {
    // 返回模拟响应
...
请配置 Gemini API Key 后再试。`
  ...
  private getMockResources(
    ...
    const mockVideos = [
      { title: `${knowledgePointName} - 基础入门`, source: 'B站', quality: 'high' as const, description: '讲解清晰，适合零基础入门' },
      { title: `${knowledgePointName} - 进阶提高`, source: '中国大学MOOC', quality: 'high' as const, description: '系统全面，含大量例题' },
      { title: `${knowledgePointName} - 考研复习`, source: 'B站', quality: 'medium' as const, description: '针对考试重点讲解' }
    ]

    const mockDocs = [
      { title: `${knowledgePointName} 知识笔记`, source: 'GitHub', quality: 'high' as const, description: '整理精美的Markdown笔记' },
      { title: `${knowledgePointName} 公式汇总`, source: '知乎', quality: 'medium' as const, description: '核心公式与定理速查' }
    ]

    const mockPractice = [
      { title: `${knowledgePointName} 练习题库`, source: '大学学习网', quality: 'high' as const, description: '含详细解答' },
      { title: `${knowledgePointName} 历年真题`, source: '考试资源网', quality: 'medium' as const, description: '考研/期末真题汇编' }
    ]
