...
export interface ReviewState {
  ...
  dailyTimeBudget: number // 分钟
  currentMasteryVector: number[] // 各知识点掌握度
}
...
export interface OptimizationStatistics {
  ...
  projectedMastery: number // 预估最终掌握度
...
export interface Constraints {
  examDate: Date | null
  dailyTimeBudget: number // 分钟
  maxSingleSession: number // 分钟
  preferredTimeSlots: Array<{ start: number; end: number }> // 小时
 ...
 * 状态建模：构建学习状态
 */
export async function buildReviewState(userId: string, courseId?: string): Promise<ReviewState> {
  // 获取用户的知识点数据
  ...
  // 构建知识点状态
  const kpStates: KnowledgePointState[] = knowledgePoints.map(kp => {
    // 计算遗忘曲线参数（基于掌握度）
    let forgettingCurve = 0.5
    if (kp.masteryScore >= 80) forgettingCurve = 0.2
    else if (kp.masteryScore >= 60) forgettingCurve = 0.3
    else if (kp.masteryScore >= 40) forgettingCurve = 0.4
 ...
 * 动作空间定义：生成可用的复习动作
 */
export function defineActionSpace(state: ReviewState): ReviewAction[] {
  const actions: ReviewAction[] = []

  for (const kp of state.knowledgePoints) {
    // 跳过已掌握的知识点
    if (kp.masteryScore >= 90) continue
    ...
    // 难度因子：难点优先
    if (kp.difficulty === 'hard') priority += 20
    else if (kp.difficulty === 'medium') priority += 10
 ...
 */
export async function optimizeReviewPlan(userId: string, options: {
  courseId?: string
...
function generateOptimizedSchedule(
  ...
  for (let day = 0; day < maxDays && remainingActions.length > 0; day++) {
    const daySessions: ReviewSession[] = []
    let dayMinutes = 0
    ...
    for (let sessionIdx = 0; sessionIdx < timeSlots.length; sessionIdx++) {
      ...
      if (dayMinutes >= dailyTimeBudget) break
 ...
 * 计算预估最终掌握度
 */
function calculateProjectedMastery(state: ReviewState, schedule: DailySchedule[]): number {
  let totalGain = 0
...
function analyzeFeasibility(
  ...
  const riskLevel: 'low' | 'medium' | 'high' = issues.length === 0 ? 'low' : issues.length === 1 ? 'medium' : 'high'
 ...
 * 获取学习效率分析
 */
export async function getEfficiencyAnalysis(userId: string, courseId?: string) {
  const state = await buildReviewState(userId, courseId)
