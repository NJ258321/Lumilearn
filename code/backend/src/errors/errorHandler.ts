...
import { AppError, ValidationError, AuthError, ForbiddenError, NotFoundError, DatabaseError, AIServiceError, FileError } from './AppError.js'
import { ErrorCode, isKnownErrorCode, ErrorCodeToStatus, ErrorCodeToMessage } from '../types/errors.js'
...
function handlePrismaError(error: Prisma.PrismaClientKnownRequestError): AppError {
  // P2025: 记录未找到
  if (error.code === 'P2025') {
    const modelName = error.meta?.modelName as string || '记录'
  ...
  if (error.code === 'P2002') {
    ...
    return new AppError(
      ErrorCode.NOT_FOUND, // 使用通用错误码，实际应该是冲突
      `字段 ${field.join(', ')} 已存在`
    )
  }
 ...
 * 处理 Multer 错误
 */
function handleMulterError(error: any): FileError {
  switch (error.code) {
