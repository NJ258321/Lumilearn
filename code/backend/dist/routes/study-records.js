...
router.post('/', [
    ...
    try {
        const { courseId, chapterId, title, date, audioUrl, duration, status = 'RECORDING', notes = '' } = req.body;
        // 验证课程是否存在
...
router.post('/:id/time-marks', [
    ...
    try {
        const { id } = req.params;
        const { type, timestamp, knowledgePointId, pptPage, content, imageUrl } = req.body;
        // 验证学习记录是否存在
        const studyRecord = await prisma.studyRecord.findUnique({
            where: { id: id }
        });
...
router.post('/:id/emphasis', [
    ...
    try {
        const { id } = req.params;
        const { timestamp, content, pptPage, imageUrl, knowledgePointId } = req.body;
        // 验证学习记录是否存在
        const studyRecord = await prisma.studyRecord.findUnique({
            where: { id }
        });
...
router.post('/:id/notes-with-timestamp', [
    ...
    try {
        const { id } = req.params;
        const { timestamp, content, knowledgePointId } = req.body;
        // 验证学习记录是否存在
        const studyRecord = await prisma.studyRecord.findUnique({
            where: { id }
        });
...
router.get('/:id/related-marks', [
    ...
    try {
        const { id } = req.params;
        const range = parseInt(req.query.range) || 30; // 默认30秒范围
        // 验证学习记录是否存在
        const studyRecord = await prisma.studyRecord.findUnique({
            where: { id },
            include: {
