        ...
        if (format === 'json') {
            res.json({
                ...
                data: {
                    ...
                    statistics: {
                        totalRecords,
                        breakdown: Object.fromEntries(dataTypes.map(type => [type, exportData[type]?.length || 0]))
                    },
                    data: exportData
                }
            });
...
router.post('/export/report', [
    ...
    try {
        ...
        if (include.includes('progress')) {
            ...
            const masteredPoints = chapters.reduce((sum, ch) => sum + ch.knowledgePoints.filter(kp => kp.status === 'MASTERED').length, 0);
            reportData.progress = {
