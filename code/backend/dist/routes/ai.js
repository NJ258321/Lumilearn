...
router.post('/ai/suggest', [
    ...
    try {
        const { courseId, targetGrade = 'B', daysUntilExam = 30, dailyStudyTime = 120, focusAreas = ['weak_points'] } = req.body;
        // 获取课程信息
...
router.post('/ai/analyze-weak-points', [
    ...
    try {
        ...
        const analyzedPoints = knowledgePoints.map(kp => {
            ...
            // 计算未复习天数
            const daysSinceReview = Math.floor((Date.now() - new Date(kp.updatedAt).getTime()) / (1000 * 60 * 60 * 24));
...
router.post('/ai/search-resources', [
    ...
    try {
        const { knowledgePointId, resourceTypes = ['video', 'document', 'practice'], language = 'zh-CN', maxResults = 5 } = req.body;
        // 获取知识点信息
        const knowledgePoint = await prisma.knowledgePoint.findUnique({
            where: { id: knowledgePointId },
            include: {
