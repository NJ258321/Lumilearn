...
router.get('/statistics/course/:courseId/overview', [
    ...
    try {
        ...
        const averageMasteryScore = totalKnowledgePoints > 0
            ...
            : 0;
        // 按天统计学习时长
        const studyTimeByDayMap = new Map();
...
router.get('/statistics/activities', async (_req, res) => {
    try {
        // 获取各类活动数量
        const [studyRecordsCount, notesCount, timeMarksCount, emphasisCount, mistakesCount] = await Promise.all([
...
router.get('/statistics/dashboard', async (_req, res) => {
    try {
        // 获取所有课程概览
        const courses = await prisma.course.findMany({
            include: {
