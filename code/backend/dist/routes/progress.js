...
router.get('/progress/overview', async (_req, res) => {
    try {
        // 获取所有课程及其进度
        ...
        const courseProgress = courses.map(course => {
            ...
            const completedPoints = course.chapters.reduce((sum, ch) => sum + ch.knowledgePoints.filter(kp => kp.status === 'MASTERED').length, 0);
