...
// ==================== PLAN-03: 多学科统筹算法 ====================
export async function optimizeMultiCourse(userId, request) {
    ...
    const courseAllocations = courses.map(course => {
        ...
        // 确定优先级
        let priority = 'medium';
        if (urgencyScore >= 60 || daysUntilExam <= 14)
            priority = 'high';
        else if (urgencyScore < 30 && daysUntilExam > 60)
            priority = 'low';
        return {
            courseId: course.id,
...
function generateDailyMultiCourseSchedule(allocations, preferences) {
    const schedule = [];
    const daysToShow = 7;
    const timeSlots = [
    ...
    for (let d = 0; d < daysToShow; d++) {
        ...
        for (const allocation of sortedAllocations) {
            if (timeSlotIndex >= timeSlots.length)
                break;
            if (totalHours >= allocations.reduce((s, a) => s + a.dailyHours, 0))
                break;
            ...
            if (hoursRemaining <= 0)
                continue;
            // 检查偏好
            if (preferences.morningCourse && slot.type !== 'morning')
                continue;
            if (preferences.eveningCourse && slot.type !== 'evening')
                continue;
            sessions.push({
                timeSlot: slot.time,
...
// ==================== PLAN-04: 任务智能平摊算法 ====================
export async function balanceTasks(userId, request) {
    ...
    // 计算任务优先级
    const priorities = reviewPlans.map(plan => {
        const course = courseMap.get(plan.courseId);
        const kp = kpMap.get(plan.knowledgePointId);
    ...
    for (const [dateStr, count] of dailyTaskCount) {
        if (count > maxDailyTasks) {
            const dateTasks = reviewPlans.filter(p => new Date(p.reviewDate).toISOString().split('T')[0] === dateStr);
            // 将低优先级任务移到后续日期
...
//# sourceMappingURL=multiCourse.js.map