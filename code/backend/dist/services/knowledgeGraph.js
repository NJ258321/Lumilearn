...
export async function detectWeaknesses(options) {
    ...
    // 检测学习路径断裂
    const orphanedPoints = knowledgePoints.filter(p => p.incomingRelations.length === 0 &&
 ...
 * 检测知识图谱的完整性问题
 */
export async function detectGraphGaps(courseId) {
    const knowledgePoints = await prisma.knowledgePoint.findMany({
        where: courseId ? { chapter: { courseId } } : undefined,
        include: {
 ...
 * 基于图结构分析，推荐最优学习顺序
 */
export async function getLearningPath(knowledgePointId) {
    // 获取目标知识点
    ...
    // 获取所有前置知识点（递归）
    const prerequisites = [];
    async function getAllPrereqs(pointId) {
 ...
 * 基于：被引用次数 * 重要性权重 + 难度系数
 */
export async function calculateImportance(knowledgePointId) {
...
export async function analyzeGraphPriority(options) {
    ...
    for (const point of knowledgePoints) {
        ...
        // 综合优先级分数
        const priorityScore = Math.min(100, Math.round(masteryFactor * 40 + // 掌握度权重 40%
    ...
    // 快速胜利洞察
    const quickWins = priorities.filter(p => p.priorityLevel === 'low' && p.masteryScore >= 80);
    ...
    // 基础点洞察
    const foundationPoints = priorities.filter(p => p.impactRadius >= 3 && p.masteryScore < 60);
 ...
 */
function calculateInfluenceScores(graph) {
