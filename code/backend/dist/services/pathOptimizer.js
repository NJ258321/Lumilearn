...
export async function optimizeLearningPath(userId, request) {
...
async function buildLearningState(userId, courseId) {
...
function calculateMasteryProjection(state, schedule, targetMastery) {
    ...
    for (let i = 2; i < schedule.length; i += 3) {
        ...
        accumulatedGain += day.sessions.reduce((s, session) => s + session.expectedScoreGain, 0);
...
export async function optimizeSinglePointPath(knowledgePointId, targetMastery = 90) {
