...
async function processReminder(reminder) {
    try {
        ...
        if (recurringConfig?.enabled && recurringConfig.pattern) {
            const nextScheduledAt = calculateNextScheduledTime(new Date(reminder.scheduledAt), recurringConfig.pattern, recurringConfig.daysOfWeek);
            if (nextScheduledAt) {
                // 更新为下一次提醒时间
 ...
 * 计算下一次提醒时间
 */
function calculateNextScheduledTime(currentTime, pattern, daysOfWeek) {
 ...
 * 根据用户设置自动创建每日学习提醒
 */
export async function generateDailyStudyReminder(userId, reminderTime = '09:00') {
    try {
 ...
 * 根据复习计划生成复习提醒
 */
export async function generateReviewReminders(userId) {
    try {
        // 获取今日需要复习的内容
        const today = new Date();
