...
import { ChevronRight, PlayCircle, Zap, Trophy, FileText, ChevronLeft, Loader2 } from 'lucide-react';
import { AppView } from '../types';
import { mistakesApi, type Mistake, type WeakPoint } from '../services/api';

interface DrillProps {
  onNavigate: (view: AppView) => void;
}

// Transform weak points to mastery data
const calculateMasteryData = (weakPoints: WeakPoint[]) => {
  ...
  const weak = weakPoints.filter(wp => wp.masteryScore < 40).length;
  const moderate = weakPoints.filter(wp => wp.masteryScore >= 40 && wp.masteryScore < 70).length;
  const mastered = weakPoints.filter(wp => wp.masteryScore >= 70).length;
  const total = weakPoints.length;
  ...
  return [
    { name: '薄弱', value: Math.round((weak / total) * 100), color: '#EF4444' },
    { name: '待巩固', value: Math.round((moderate / total) * 100), color: '#F59E0B' },
    { name: '已掌握', value: Math.round((mastered / total) * 100), color: '#3B82F6' },
  ];
};
...
const Drill: React.FC<DrillProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch mistakes and weak points in parallel
        const [mistakesResponse, weakPointsResponse] = await Promise.all([
          mistakesApi.getAll(),
          mistakesApi.getWeakPoints()
        ]);

        if (mistakesResponse.success && mistakesResponse.data) {
          setMistakes(mistakesResponse.data);
        }

        if (weakPointsResponse.success && weakPointsResponse.data) {
          setWeakPoints(weakPointsResponse.data);
        }
      } catch (err) {
  ...
  const masteryData = calculateMasteryData(weakPoints);
  const overallMastery = masteryData.find(d => d.name === '已掌握')?.value || 0;
  const weakCount = masteryData.find(d => d.name === '薄弱')?.value || 0;
              ...
              <div className="flex items-center">
                  ...
                  <div className="relative w-32 h-32 flex-shrink-0">
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                          <div className="text-2xl font-black text-slate-800 leading-none">{overallMastery}<span className="text-xs font-bold text-emerald-500 ml-0.5">%</span></div>
                      ...
                      <div className="space-y-2">
                          ...
                          <div className="flex items-center justify-between group cursor-pointer active:opacity-70">
                              ...
                              <div className="flex items-center space-x-2">
                                  <span className="text-xs text-slate-400 font-bold">{weakCount}%</span>
                          ...
                          <div className="flex items-center justify-between group cursor-pointer active:opacity-70">
                              ...
                              <div className="flex items-center space-x-2">
                                  <span className="text-xs text-slate-400 font-bold">{masteryData.find(d => d.name === '待巩固')?.value || 0}%</span>
                                  <ChevronRight size={14} className="text-slate-300 group-hover:text-blue-500 transition-colors" />
                              </div>
                          </div>
                          {/* 列表项 3 */}
                          <div className="flex items-center justify-between group cursor-pointer active:opacity-70">
                              ...
                              <div className="flex items-center space-x-2">
                                  <span className="text-xs text-slate-400 font-bold">{overallMastery}%</span>
                                  <ChevronRight size={14} className="text-slate-300 group-hover:text-blue-500 transition-colors" />
                              </div>
                          </div>
                      </div>
          ...
          <div className="bg-white rounded-t-2xl shadow-[0_-5px_20px_rgba(0,0,0,0.02)] border border-slate-100 -mx-4 px-6 pt-6 pb-4">
              ...
              {mistakes.length === 0 ? (
                ...
                <div className="space-y-6">
                  {mistakes.slice(0, 5).map((mistake) => (
                    <div key={mistake.id} className="relative pl-4 border-l-2 border-slate-100">
                        <div className="flex items-start mb-2">
                            ...
                            <div className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded text-center">
                                {mistake.knowledgePoint?.name || '未知知识点'}
                            </div>
                        </div>

                        <div className="mb-3">
                            <h3 className="text-sm font-bold text-slate-800 mb-2 leading-relaxed">
                                <span className="text-red-500 mr-1">A.</span> {mistake.question.substring(0, 50)}...
                            </h3>
                            <p className="text-xs text-slate-500 leading-relaxed mb-2">
                                {mistake.question.substring(50, 150)}...
                            </p>
                            <p className="text-xs text-slate-400 font-mono">
                                标准作答：{mistake.correctAnswer}
                            </p>
                        </div>
              ...
              {mistakes.length > 5 && (
                <button className="w-full mt-6 py-3 text-xs font-bold text-blue-500 flex items-center justify-center space-x-1 active:bg-blue-50 rounded-xl transition-colors">
                    <span>查看全部 {mistakes.length} 道错题</span>
                    <ChevronRight size={14} />
                </button>
              )}
          </div>
